        ...
        internal override void DumpConstants(Emit.Function func, WriterOptions options)
        {
            ...
            for (int i = 0; i < func.ConstantCount; i++)
            {
                ...
                if (con.AltType.HasValue)
                    writer.Write((byte)con.AltType.Value);
                ...
                switch (con.Type)
                {
                    ...
                    case ConstantType.NUMBER:
                        ...
                        {
                            if (Is64Bit)
                                writer.Write(DoEndian(BitConverter.GetBytes((long)con.Value)));
                            else writer.Write(DoEndian(BitConverter.GetBytes((int)con.Value)));
                        }
                        else
                        {
                            if (Is64Bit)
